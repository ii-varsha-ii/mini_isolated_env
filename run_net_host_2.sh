...
# Setup network namespaces for HOST - Use the same script to create network connections in different host
...
CAP_ADDR="192.168.3.11/24 192.168.3.12/24 192.168.1.13/24 192.168.1.14/24 192.168.1.15/24"

BRIDGE_ADDR_WITH="192.168.3.10/24"
BRIDGE_ADD="192.168.3.10"
...
sudo ip link add name br1 type bridge || true
...
do
  # bridge setup
  sudo ip -all netns exec ip route add default via "${BRIDGE_ADD}"
done
